...
const isTabLayout = (areas: RichMenuPayload['areas']) => {
  const first = areas[0]?.bounds
  const second = areas[1]?.bounds
  return Boolean(first && second && first.x === 0 && first.y === 0 && second.y === first.height && first.width > second.width)
}
        ...
        <div className="w-full overflow-hidden rounded-xl bg-white shadow-lg">
          <div className="relative bg-slate-200" style={{ aspectRatio: `${width} / ${height}` }}>
            ...
            {menu.areas.map((area, index) => {
              const isTab = tabLayout && index === 0
              const label = tabLayout ? index + 1 : index + 1
